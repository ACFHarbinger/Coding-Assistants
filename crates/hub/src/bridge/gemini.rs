...
pub fn start_gemini_managed_harness(
    ...
    (
        ...
        crate::HarnessSessionRegistration,
    ),
    String,
> {
    start_gemini_managed_harness_with(store, workspace, prompt, run_agy_worker)
}

/// Testable core: takes the `agy` worker runner as a parameter so tests
/// never shell out to a real `agy` binary. Mirrors `deliver_gemini_task_with`.
pub fn start_gemini_managed_harness_with(
    store: &HubStore,
    workspace: &Path,
    prompt: &str,
...
> {
    ...
    let (pid, output) = runner(&workspace, prompt, None)?;
