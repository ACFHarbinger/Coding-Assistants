...
- **`allow_metered_quota_probes` setting (Settings → Orchestration, default on)**: most provider-usage adapters read a free metadata endpoint (`claude` hits the same `/api/oauth/usage` route `/usage` renders from; `cursor`/`grok`/`codex`/`deepseek` use first-party endpoints or `codex app-server`), but three have no free surface — `gemini` (`agy --print "/usage"`) and `opencode` (`opencode run "/ogc-usage"`) cost a model turn, and `muse` costs one minimal completion (~24 tokens). The new global-only boolean gates just those three: when off they report `status: "unavailable"` with a detail pointing back at the setting, while every other provider is unaffected. Default is on so a bare install still checks every provider; a distributor turns it off before handing the app to testers (an install wizard to prompt for defaults like this is planned). Global-only because quota is an account-level concept — no workspace override, hence no `_status` pill on `EffectiveOrchestrationPolicy.allow_metered_quota_probes`. Gate is threaded as an `allow_metered: bool` arg into `gemini_quota`/`opencode_quota`/`muse_quota` and read once per refresh in both `hub_get_provider_quotas` and `hub_refresh_provider_quota`. `cargo test -p hub` (319), `cargo test -p tauri-app` (173 + quota adapters), `cargo clippy -p hub -p tauri-app`, and `npm test` (92) all pass.
- **Background usage-refresh settings (`quota_auto_refresh_enabled` / `quota_auto_refresh_interval_secs`, Settings → Orchestration)**: the Usage strip's background poll is now user-controlled and **off by default** — a timed refresh spends tokens on the metered adapters (`gemini`/`opencode`/`muse`) with no user in the loop, so a fresh install stays quiet until opted in. When on, the cadence is `quota_auto_refresh_interval_secs`, range-checked to 30–3600s (default 300s) by `OrchestrationPolicy::validate`. Both fields are global-only (no workspace override, no `_status`), added to `OrchestrationPolicy` / `EffectiveOrchestrationPolicy` / the `OrchestrationPatch` IPC and mirrored in `src/components/settings/types.ts`. Backend + contract only this slice; the `OrchestrationTab` controls and the `QuotaStatusStrip` gating are assigned to Gemini (see `.agent/cache/AGENT_BUS.md`, 2026-09-09). `cargo test -p hub` (320), `cargo test -p tauri-app --lib` (173), `cargo clippy -p hub -p tauri-app`, `npx tsc`, and `npm test` (92) all pass.
- **Codex turn-completion replies (C14.2, #149, `1633837`)**: Hub-delivered Codex tasks now wait for the matching `turn/completed` app-server notification and route its final `agentMessage` text back to the original sender; session replies use unique subjects. This fixes the missing-reply gap but does **not** add #149's persistent per-thread app-server daemon/broker, whose control-socket framing remains an open follow-up. `cargo build --workspace` and `cargo clippy --workspace --all-targets` are clean; no tests were run.
- **Claude Channel automated acceptance coverage (C14.3, #150, `b95baaf`)**: added an isolated HubStore-level acceptance round trip for the opt-in Claude Code Channel bridge. It checks the enrolled-sender task path, quiet-versus-interrupting delivery with ack-on-drain, replies returning to the original sender, and never-auto-approved permission relay. `cargo build --workspace` and `cargo check -p hub --tests` compile clean; no tests were executed under the machine's no-test-suite constraint.
- **Memory graph links + heuristic link-suggestion matcher (M7, #159)**: memories can now be linked to each other directly (`memory_links`, a directed edge with freeform `relation` and mandatory `created_by` provenance), not just to their originating source event — `link/unlink/links/related/topic` CLI/IPC commands, plus a dependency-free tag+token similarity matcher (`suggest-links`/`apply-suggestions`) that scores candidate connections and, under a new `off`/`suggest`/`auto` `LinkSuggestionMode` setting, can auto-draw high-confidence edges (attributed to the system, never an agent's own author field). See `docs/moon/CHANGELOG.md` for the full detail, including a real threshold recalibration from a smoke test. **Frontend UI landed same day** — a per-memory links panel with suggest-and-link, plus a "Browse by topic" view grouping memories by agent side by side. M7 is now closed end to end.
