...
- **Chat & Memory sort order toggle**: a header button next to the search box lets the human dev flip the active channel/DM/work-session stream between ascending (oldest first, default) and descending (newest first) date-time order (`MessagerPanel.tsx`'s `sortOrder` state, rendered in `messager/ChatHeader.tsx`). Purely a client-side view toggle — no backend or storage change.
