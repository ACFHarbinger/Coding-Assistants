...
| H7 | Per-user linked external accounts: a user links an external provider account (ChatGPT / Claude / Google / …) — OAuth device flow or a recorded "uses the vendor CLI's own native login" — and the shared Hub stores which provider identities *that user* holds, keyed to the H2 namespaced identity, never global. The linked-account record holds only `{ provider, external_account_label, connection_kind, linked_at, token_ref }`; the token itself lives in the `platform.md` P12 vault, not in `hub.db`. Unlinking revokes the vault entry and the record together. | A second human on the same Hub links their own ChatGPT/Google account without seeing, using, or colliding with the first human's; every provider call and quota read resolves the *caller's* linked account; a Hub with one user behaves exactly as today | 🔬 Research · **blocked on H2** — the record's primary key is the namespaced identity H2 defines. [#286](https://github.com/ACFHarbinger/Coding-Assistants/issues/286) (2026-09-08 credentials batch, parent [#279](https://github.com/ACFHarbinger/Coding-Assistants/issues/279)) opens this for **design only**: until H2 lands, `settings.md` S8 (#284) stores links single-user under a provisional `local` owner key that the H2 migration will rewrite. No implementer assigned this round. |
