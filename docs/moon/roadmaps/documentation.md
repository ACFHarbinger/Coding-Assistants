...
| --- | --- | --- |
| Deployed docs | GitHub Actions builds the React static site and deploys `docs/website/dist` to Pages | Keep the locked Node/content-build/Vite deployment flow and production acceptance checks. |
| Website | `docs/website` is a React 19 + TypeScript + Vite application with modular feature components and HashRouter | Maintain desktop-matched tokens and a componentized, testable information architecture. |
| Content | `docs/` Markdown is canonical; `scripts/build-content.ts` builds typed content, MiniSearch, and navigation metadata | Keep generated artifacts ignored and rebuild them before test/build. |
| Branding | Coding-Assistants metadata, favicon, and social metadata are in the React site | Keep naming, footer/legal, and social metadata consistent with the desktop product. |
...
| Tests | Vitest unit and integration suites under `docs/website/tests/`; Cypress smoke/E2E flows under `tests/cypress/`. Validate the content pipeline, reader/landing invariants, HashRouter navigation, local search, and theme persistence. Built-output suites run serially with an explicit clean-runner-safe hook budget. |
...
| --- | --- | --- | --- |
| W1 | Foundation and React setup | Replace Vue tooling with React 19 + TypeScript + Vite in isolated `docs/website`. HashRouter, Tailwind configured with desktop design tokens, self-hosted fonts, Vite Pages base. | ✅ **Done** · React/Vite, HashRouter, Tailwind desktop tokens, local fonts, before-paint theme boot, and the locked Markdown renderer are in place. |
...
| W3 | Documentation reading experience | Glass sidebar, `react-markdown` article, scroll-aware TOC, heading anchors, prev/next, code copy, syntax highlighting, Mermaid.js. | ✅ **Done (Chat/Codex)** · The public Pages acceptance pass confirms shared indigo/theme-token reader chrome, safe Markdown code rendering, and error recovery for unknown documentation slugs. |
| W4 | Product landing page | Product-forward landing: hero with the abstract multi-agent architecture graphic, quick-start snippet, capability grid, docs + GitHub CTAs. Docs reader stays one click away. | ✅ **Done (Grok/Chat)** · Public Pages renders the landing, Hub graphic, navigation, CTAs, and required Messager wording; the latter is regression-tested. |
| W5 | Search palette & theme toggle | `Cmd+K` / `Ctrl+K` MiniSearch palette; Dark/Light (optional System) toggle with persisted choice and zero theme flash. | ✅ **Done (Grok/Chat)** · Public dark-first chrome and search affordance passed inspection; local Cypress verifies route navigation and persisted theme selection. |
| W6 | CI/CD Pages deployment & cutover | Update `.github/workflows/docs.yml` to `npm ci` in `docs/website`, run content build + Vite, deploy `dist/`. Update contributor docs; retire MkDocs and `generate_docs_json.py`. | ✅ **Done (Chat/Codex)** · Workflow-backed Pages deployments, including final clean-runner verification `31676870915`, passed build, test, artifact upload, deployment, and public rendered-DOM acceptance. The inactive MkDocs config and legacy JSON generator are retired; Git history remains the rollback path. |
| W7 | Polish | Social cards, recolored-mark favicon set, skip-link audit, mobile drawer, print stylesheet for articles, 404, AGPL footer. Optional later: Docker preview aligned with I1. | ✅ **Done (Claude/Chat)** · Privacy/a11y checks, release checklist, social metadata, favicon, footer, skip link, mobile drawer, print stylesheet, and custom 404 are deployed and verified. Unit/integration/Cypress coverage is organized under `tests/`; the dedicated social-card asset remains optional. |
