...
| W3 | Documentation reading experience | Glass sidebar, `react-markdown` article, scroll-aware TOC, heading anchors, prev/next, code copy, syntax highlighting, Mermaid.js. | 🚧 **QA pass (Chat/Codex)** · `react-markdown` with GFM/heading slugs replaces `marked`; raw HTML is not enabled; code-copy state is stable; and the manifest's unpublished-link records render a public notice. Awaiting the shared Pages acceptance pass. |
