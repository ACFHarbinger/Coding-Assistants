...
| W6 | CI/CD Pages deployment & cutover | Update `.github/workflows/docs.yml` to `npm ci` in `docs/website`, run content build + Vite, deploy `dist/`. Update contributor docs; retire MkDocs and `generate_docs_json.py`. | 🚧 **Deployed (Chat/Codex)** · Workflow-backed Pages is enabled; commit `9fa3bce` completed build, test, artifact upload, and deployment, and the public URL returns HTTP 200. Await repair/deployment of remaining W3 reader regressions before legacy-file retirement and closure. |
