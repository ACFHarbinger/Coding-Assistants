...
| W4 | Product landing page | Product-forward landing: hero with the abstract multi-agent architecture graphic, quick-start snippet, capability grid, docs + GitHub CTAs. Docs reader stays one click away. | 🚧 **QA pass (Grok)** · Landing, Hub graphic, brand mark, and theme-aware cards are in place. Cyan chrome removed from owned landing files. Owner Pages visual check still open. |
| W5 | Search palette & theme toggle | `Cmd+K` / `Ctrl+K` MiniSearch palette; Dark/Light (optional System) toggle with persisted choice and zero theme flash. | 🚧 **QA pass (Grok)** · Palette, Dark/Light/System, mobile drawer, reduced-motion, and indigo nav chrome hardened. Docs-reader cyan left to W3. |
| W6 | CI/CD Pages deployment & cutover | Update `.github/workflows/docs.yml` to `npm ci` in `docs/website`, run content build + Vite, deploy `dist/`. Update contributor docs; retire MkDocs and `generate_docs_json.py`. | 🚧 **In review (Chat/Codex)** · React build/test/deploy workflow and cutover/rollback guidance are in place; a clean archive passes the exact CI command sequence. Pages owner verification must precede legacy-file retirement and closure. |
| W7 | Polish | Social cards, recolored-mark favicon set, skip-link audit, mobile drawer, print stylesheet for articles, 404, AGPL footer. Optional later: Docker preview aligned with I1. | 🚧 **Partial (Claude)** · privacy/a11y regression checks, release checklist, social metadata, favicon, footer, skip link, and mobile drawer are in place. Print styling, a custom 404, dedicated social-card asset, and Pages acceptance remain. |
