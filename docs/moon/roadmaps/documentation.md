...
| W6 | CI/CD Pages deployment & cutover | Update `.github/workflows/docs.yml` to `npm ci` in `docs/website`, run content build + Vite, deploy `dist/`. Update contributor docs; retire MkDocs and `generate_docs_json.py`. | ✅ **Done (Chat/Codex)** · Workflow-backed Pages deployments at commits `9fa3bce` and `67c359b` passed build, test, artifact upload, deployment, and public rendered-DOM acceptance. The inactive MkDocs config and legacy JSON generator are retired; Git history remains the rollback path. |
