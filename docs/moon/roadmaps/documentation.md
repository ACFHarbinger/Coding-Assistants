...
### 2026-08-13 — Grok (Owner aesthetic / architecture / implementation lock)

Harbinger answered Grok's follow-up questions. The following **supersede** conflicting Gemini locks above (status remains Approved).

- **Palette:** Use the **live desktop** tokens (`#020617`, indigo `#6366f1`, purple `#a855f7`, glass cards, two-corner glows). Do **not** use the earlier cyan `#24C8D8` / violet `#8B5CF6` pair.
- **Atmosphere:** Dark-first, desktop HUD glass; light is a toggle. Honor `prefers-reduced-motion`.
- **IA:** Product-forward landing at `/#/`, docs reader at `/#/docs`. Nested slugs. Project Status nav now includes Cloud Sync and this Documentation roadmap.
- **Corpus:** Curated publish set (`docs/*.md`, `docs/adr/**`, Moon ROADMAP/CHANGELOG, `docs/moon/roadmaps/*.md`). Exclude archive, research, and reports from sidebar/search.
- **Stack:** Isolated `docs/website` npm app; `react-markdown` + `remark-gfm`; MiniSearch (not FlexSearch/Fuse); self-hosted Inter / JetBrains Mono (no Google Fonts); gitignored generated JSON; Vitest + Testing Library; no analytics/cookies; AGPL footer; 500-line file bound; copy tokens, do not import desktop `src/`.
- Added implementation-lock table, frontmatter/slug rules, W7 polish, token table, privacy/validation rows, and infra I1 note.
- Recorded that the Vue prototype still uses Plus Jakarta Sans and that desktop vs earlier site-lock accents had diverged.

### 2026-08-13 — Claude (Styling, syntax highlighting, hero visual, brand mark Q&A)

Harbinger answered four open implementation/aesthetic questions the roadmap
hadn't resolved yet (status remains Approved; nothing here reopens a prior
lock, it fills gaps left by Gemini/Grok's passes).

- **Styling implementation:** Tailwind CSS, `theme.extend` populated from
  the desktop token table, rather than hand-written CSS or CSS Modules —
  a deliberate divergence from the desktop app's plain-CSS approach for
  build velocity across many small components. Token *values* are
  unaffected; the 500-line bound still applies per-component file.
- **Syntax highlighting:** `react-syntax-highlighter` (Prism themes),
  lazy-loaded per language, rather than Shiki or `rehype-highlight`.
- **Landing hero visual:** An abstract multi-agent architecture graphic
  (radial glow, Hub node, Grok/Claude/Codex/Gemini satellite nodes) rather
  than a desktop app screenshot — stays visually current with no upkeep
  and doubles as an architecture-docs preview.
- **Brand mark:** Recolor the existing desktop app icon
  (`src-tauri/icons/`, an interlocking-circles mark currently cyan/gold)
  into the site's indigo `#6366f1` / purple `#a855f7` tokens for the
  header logo and favicon, kept as SVG source under
  `docs/website/src/assets/brand/` — rather than a new mark or a
  text-only wordmark.
- Updated the technical architecture diagram, implementation-lock table,
  source layout tree, and W1/W4/W7 delivery-phase rows to reflect all four
  decisions.
