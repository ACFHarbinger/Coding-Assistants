...
| W3 | Documentation reading experience | Glass sidebar, `react-markdown` article, scroll-aware TOC, heading anchors, prev/next, code copy, syntax highlighting, Mermaid.js. | 🚧 **Ready for deployment (Chat/Codex)** · Reader chrome now uses the shared indigo/theme-token system; raw HTML remains disabled; React Markdown internals are not emitted to the DOM; unknown document slugs use the error page; and reader regressions are covered. Deploy and rerun public acceptance. |
| W4 | Product landing page | Product-forward landing: hero with the abstract multi-agent architecture graphic, quick-start snippet, capability grid, docs + GitHub CTAs. Docs reader stays one click away. | 🚧 **Pages QA pass (Grok/Chat)** · Landing, Hub graphic, navigation, and CTAs render correctly on public Pages. The last legacy “Slack-like” label was replaced with Messager and is regression-tested. |
| W5 | Search palette & theme toggle | `Cmd+K` / `Ctrl+K` MiniSearch palette; Dark/Light (optional System) toggle with persisted choice and zero theme flash. | 🚧 **Pages QA pass (Grok/Chat)** · Public header renders the dark-first theme controls and keyboard-search affordance; local search/theme/drawer checks remain green. |
...
| W7 | Polish | Social cards, recolored-mark favicon set, skip-link audit, mobile drawer, print stylesheet for articles, 404, AGPL footer. Optional later: Docker preview aligned with I1. | 🚧 **Ready for deployment (Claude)** · privacy/a11y regression checks, release checklist, social metadata, favicon, footer, skip link, mobile drawer, print stylesheet, and custom 404 are complete. Dedicated social-card asset remains optional; deploy alongside the W3 repair and complete the shared checklist. |
