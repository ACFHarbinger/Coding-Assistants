...
| C12 | Bidirectional harness capture and inject | The app captures messages agents send inside Grok/Chat/Claude/Gemini harnesses into the session transcript. Hub messages tagged task and/or wake are injected into the target harness so the agent executes them. Builds on C9. | 🚧 **In Progress** · Capture polls all four. Grok **tasks** deliver through the registered ACP leader bridge (`session/load` + `session/prompt` on `leader.sock`); missing leader → `unavailable`, task stays queued. Claude/Gemini/Codex active-session bridges still assigned. Wake may still spawn. |
