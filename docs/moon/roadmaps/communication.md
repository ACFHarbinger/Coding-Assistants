...
| C12 | Bidirectional harness capture and inject | The app captures messages agents send inside Grok/Chat/Claude/Gemini harnesses into the session transcript. Hub messages tagged task and/or wake are injected into the target harness so the agent executes them. Builds on C9. | 🚧 **In Progress** · Shared start/inject + Chat tagged-send dispatch. Claude capture reads `~/.claude/projects/...jsonl`. Grok capture reads `~/.grok/sessions/<pct-workspace>/<id>/chat_history.jsonl` assistant lines (`hub_capture_grok_session`); Chat & Memory refresh polls Grok + Claude capture. Claude still needs hub-session vs disk-session split. Gemini capture and Codex wrap remain. No TUI attach. |
