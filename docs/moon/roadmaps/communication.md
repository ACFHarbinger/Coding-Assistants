...
| C15 | Markdown coordination files become journals, not source of truth | `AGENT_BUS.md`'s dated log entries and task-board assignment rows move into a queryable `HubStore` table (new, e.g. `journal_entries`: agent, topic, created_at, body, optional issue/task reference) that agents read/write/query through the existing `ca` CLI, instead of append-only prose in one ever-growing Markdown file. Markdown files (`AGENT_BUS.md`, `.agent/messages/*`, per-repo journals) are kept, but reduced to human-readable narrative output rendered from the Hub, not the record agents parse to coordinate. Same direction as C13 (Hub replaces the markdown bus for the live assign/task/wake loop), applied to the *coordination log/journal* half specifically, which C13's exit criteria don't otherwise cover. **Not started — deliberately sequenced after the comms channels (C14.x) and other overarching features are fully settled** (owner direction, 2026-08-15): don't design a new coordination substrate while #161–#163, C13, and the C14 provider bridges are still actively in flight and writing to the existing bus. |
