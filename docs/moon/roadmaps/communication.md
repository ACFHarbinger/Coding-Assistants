...
| C14.13 | Alibaba **Qwen Code** harness | Onboard the Qwen Code CLI (`qwen`, `0.23.2`), a Gemini-CLI fork, as a managed harness. **CLI contract captured live 2026-09-09 — see the dated note below.** Its transcript is the **Claude-Code JSONL format** (`~/.qwen/projects/<sanitised-cwd>/chats/<sessionId>.jsonl`, `uuid`/`parentUuid`/`sessionId`/`timestamp`/`type`/`provenance`/`message.parts`), so the adapter mirrors `src-tauri/src/harness/claude.rs` / `gemini.rs`, **not** the Muse/Vibe event-log pattern. `--session-id` lets the caller pre-assign the id (Muse-style managed capture, no discover-then-register). `/coordinate` sub-agent team-runner + git-worktree isolation still need capture that does not collapse concurrent sub-agent output. Harness-only for control (Qwen Code carries its own OpenAI-compatible model config) — usage is separate: `platform.md` **P3** tracks the Qwen quota adapter ([#310](https://github.com/ACFHarbinger/Coding-Assistants/issues/310), **landed** — local `~/.qwen/usage_record.jsonl` reader; Token Plan remaining credits are console-only, no key-authenticated budget API). `HarnessId::Qwen` + `("qwen", "Qwen Code")` identity + `git/messages/qwen_coauthor.msg`. **Landed 2026-09-11 — [#308](https://github.com/ACFHarbinger/Coding-Assistants/issues/308), 1.0.0 milestone.** Grok; reviewed and merged by Claude (no live Codex session available at merge time). Owner rationale (2026-09-09 provider-menagerie review): a `/coordinate`-capable in-harness multi-agent runner to benchmark against CA's own orchestration. **Live one-shot acceptance completed 2026-09-12** (the piece not run at original landing): Grok drove the real managed-Hub path end to end (`start_managed_harness("qwen")` → capture → headless `deliver_qwen_task` resume) after the owner's API-key issue (Token Plan vs. Coding Plan mismatch) was resolved, and found a real bug doing it — headless inject reused `--session-id` on an already-created transcript, which `qwen` 0.23.3 rejects; fixed with a dedicated `--resume <uuid>` argv path (interactive relaunch already used `--resume`, headless inject did not). Codex PASS, merged `main`. **Owner desktop click-through confirmed 2026-09-12 — #308 closed.** Two more real bugs surfaced and fixed in the same pass, both affecting every harness onboarded after the original four (claude/chat/gemini/grok), not just Qwen: (a) the Provider CLI & Auth Health probe only checked the OS process environment for a configured key, never `settings.json`'s own `env` map, which is where the CLI's own setup wizard (Coding Plan / Token Plan / Standard API Key) actually writes it — every wizard-configured install misreported "needs login" (`commands/health/qwen_probe.rs`); (b) `WorkspaceAgentPresence` never carried a field for Qwen — or opencode/deepseek/vibe/muse/cursor/kimi — so the Chat & Memory presence dot stayed grey regardless of real session state (`crates/hub/src/bridge/presence.rs` + frontend `agentIsLive`). Both fixed directly on `main`, gate green. |
| C14.14 | Moonshot **Kimi** swarm harness | Onboard the Kimi Code CLI (`kimi`, `0.42.0`) as a managed harness. **CLI contract captured live 2026-09-09 — see the dated note below.** Unlike Qwen it has its **own event-log transcript** (`~/.kimi-code/sessions/wd_<slug>/session_<uuid>/agents/main/wire.jsonl` + `state.json`), closer to the Muse pattern but with distinct event types, and a real machine-readable session enumerator (`kimi session list --json`). `kimi acp` (stdio ACP server) is a clean managed-delivery transport, like Grok's leader ACP. The reason to onboard is the capability, not the model: swarm-native execution (`KIMI_CODE_AGENT_SWARM_MAX_CONCURRENCY`, `--max-subagent-depth`) is a different in-harness multi-agent design than Qwen's `/coordinate` or CA's stage graph. Harness-only for control — usage is separate: `platform.md` **P3** tracks a Kimi quota adapter ([#311](https://github.com/ACFHarbinger/Coding-Assistants/issues/311) — the local-usage half is already confirmed feasible from `wire.jsonl`'s `usage.record`; only the plan-budget half is spike-gated). `HarnessId::Kimi` + `("kimi", "Kimi")` identity + `git/messages/kimi_coauthor.msg`. **Landed 2026-09-11 — [#309](https://github.com/ACFHarbinger/Coding-Assistants/issues/309), 1.0.0 milestone.** Gemini (picked up from OpenCode's assignment); reviewed and merged by Claude (no live Codex session available at merge time). Owner rationale (2026-09-09): passed on Kimi as a plain model on cost grounds but wants the swarm-native harness as a benchmark data point. |
