...
| C4 | Configurable human gates and standing policies for wake-ups and delegation | Per-task policy can allow or require approval | 🚧 **Partial** · persisted `WakePolicy` with CLI/Tauri/desktop controls; per-task delegation policy still open |
| C5 | Declarative sequential and bounded-parallel workflow wiring | A real task can be split into plan/code/review boundaries with retries and handoffs | 🚧 **Partial** · sequential dispatch plus persisted retry/parallel-stage state scaffold; bounded dispatch and retry commands still open |
...
Wake resolution and persisted `WakePolicy` are available through CLI/Tauri and
the desktop Wakes panel; per-task delegation policy remains open. A2A
