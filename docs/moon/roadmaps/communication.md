...
| C12 | Bidirectional harness capture and inject | The app captures messages agents send inside Grok/Chat/Claude/Gemini harnesses into the session transcript. Hub messages tagged task and/or wake are injected into the target harness so the agent executes them. Builds on C9. | 🚧 **Partial** · Shared typed start/inject/capture contract + Grok spawn adapter (`ca_hub::harness`, `hub_start_harness` / `hub_inject_harness` / `hub_record_harness_capture`). Capture dedups by content hash. Claude/Gemini/Codex adapters still assigned. No TUI attach. |
