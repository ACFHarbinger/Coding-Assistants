...
| C12 | Bidirectional harness capture and inject | The app captures messages agents send inside Grok/Chat/Claude/Gemini harnesses into the session transcript. Hub messages tagged task and/or wake are injected into the target harness so the agent executes them. Builds on C9. | 🚧 **In Progress** · #145. Capture polls all four. Grok task delivery uses the provider-supported registered ACP leader path. Codex has an explicit app-server bridge when the owner supplies a persisted thread. Claude and Gemini discovery/capture are real, but their published active-session control transports are not documented; tasks therefore report `unavailable` and remain queued. No PTY writes, fabricated sockets, or replacement task processes. The desktop Hub command boundary is decomposed into sub-500-line modules without altering IPC contracts. Wake may still spawn. |
