...
| C12 | Bidirectional harness capture and inject | The app captures messages agents send inside Grok/Chat/Claude/Gemini harnesses into the session transcript. Hub messages tagged task and/or wake are injected into the target harness so the agent executes them. Builds on C9. | 🚧 **In Progress** · Shared start/inject + tagged-send dispatch. Disk capture exists for all four, each with a `disk_session_id`/`hub_session_id` split. Chat & Memory polls Grok, Claude, and Codex and reloads when new rows appear; Gemini isn't wired into that poll yet. `harness_c12.rs` is a workspace acceptance test proving fixture transcripts for all four adapters land on one shared hub session, `inject_harness` returns a structured result on bad input without spawning anything, and every harness's argv builder keeps a shell-metacharacter-laden prompt as one literal argv element. C13 remains. No TUI attach. |
