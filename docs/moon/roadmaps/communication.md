...
| C11 | Task vs wake message tags | A message may be tagged **task**, **wake**, both, or neither. **Wake** may launch a new harness instance of that identity and enroll it in the session team. **Task** must target an already-enrolled, currently present member and is refused (no spawn) otherwise. Agents can apply the same tags through the hub API/CLI. | 🚧 **Partial** · `HubStore::send_tagged_message` + `hub_send_tagged_message` + `ca msg tag` enforce task-refuse and wake-enroll, with per-recipient durable outcomes and wake-policy-aware delivery. "Currently present" = team + (session, if given) membership — no live-heartbeat signal yet. Not wired into `SlackChatPanel.tsx`'s composer, which still does a client-only task check per recipient; actual new-harness-instance launch is C12's typed provider adapter. |
