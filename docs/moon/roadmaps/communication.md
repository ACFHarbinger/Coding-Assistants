...
| C12 | Bidirectional harness capture and inject | The app captures messages agents send inside Grok/Chat/Claude/Gemini harnesses into the session transcript. Hub messages tagged task and/or wake are injected into the target harness so the agent executes them. Builds on C9. | 🚧 **In Progress** · UI polls all four capture sources; Tauri accepts camelCase tagged/session payloads; partial IPC failures are retained per target. Gemini/Antigravity uses the installed `agy` CLI. Disk capture and explicit wake spawn work, but a task cannot yet be injected into an arbitrary already-running interactive TUI; it remains durable and reports its queued state until an active-harness inbox adapter consumes it. Complete C9's registered/attached adapter path for all four harnesses before C12/C13 acceptance. |
