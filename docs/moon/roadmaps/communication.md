...
| C6 | Budget exhaustion pause, Markdown handoff summary, delegation, and shutdown | No uncontrolled provider calls continue after a configured limit | 🚧 **Partial** · `agent_budgets` blocks wakes, writes Markdown handoffs, and exposes CLI/Tauri pause/resume; provider adapters still need automatic spend reporting and shutdown hooks |
