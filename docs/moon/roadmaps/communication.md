...
| C12 | Bidirectional harness capture and inject | The app captures messages agents send inside Grok/Chat/Claude/Gemini harnesses into the session transcript. Hub messages tagged task and/or wake are injected into the target harness so the agent executes them. Builds on C9. | 🚧 **In Progress** · Shared typed start/inject/capture contract, explicit four-harness spawn adapters, capture dedup, and Chat & Memory dispatch for accepted tagged session recipients. Real harness-side capture/agent-originated sends and end-to-end live acceptance remain. No TUI attach. |
