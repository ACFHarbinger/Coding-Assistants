...
| C9 | Agent inbox bridge process | A long-lived adapter can consume one agent's hub messages as a stable stream, acknowledge them, and honor wake gates | 🚧 **In Progress** · Codex: `ca inbox watch`. Grok: registered-session ACP leader bridge (`deliver_grok_task`). Claude/Gemini active-session adapters still assigned. |
...
| C11 | Task vs wake message tags | A message may be tagged **task**, **wake**, both, or neither. **Wake** may launch a new harness instance of that identity and enroll it in the session team. **Task** must target an already-enrolled, currently present member and is refused (no spawn) otherwise. Agents can apply the same tags through the hub API/CLI. | 🚧 **In Review** · `HubStore::send_tagged_message` + `hub_send_tagged_message` + `ca msg tag` enforce task-refuse and wake-enroll, with per-recipient durable outcomes and wake-policy-aware delivery. Task-only injection is intentionally queued in the durable session inbox and never spawns a replacement agent; wake retains explicit spawn semantics. "Currently present" = team + (session, if given) membership. |
| C12 | Bidirectional harness capture and inject | The app captures messages agents send inside Grok/Chat/Claude/Gemini harnesses into the session transcript. Hub messages tagged task and/or wake are injected into the target harness so the agent executes them. Builds on C9. | 🚧 **In Progress** · Capture polls all four. Grok/Gemini **tasks** deliver through their respective ACP leader / bridge sockets (`grok_bridge.rs`, `gemini_bridge.rs`); missing socket → `unavailable`, task stays queued. Claude's bridge (`claude_bridge.rs`) does real, verified discovery via `claude agents --json` and a confirmed-real `cc-socks/<pid>.sock`, but intentionally never connects to that socket — its wire protocol is undocumented, so delivery always reports `unavailable` rather than risk a live session. Codex active-session bridge still assigned (Chat). Wake may still spawn. |
