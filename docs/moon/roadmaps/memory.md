...
| M2 | Recent raw memory plus episodic and semantic long-term tiers | Compaction preserves important decisions and links each compressed memory to source events | 🚧 In Progress · `MemoryTier` (short_term/episodic/semantic) + scope/tier/workspace filtering + substring search implemented and tested; **compaction/promotion pipeline not implemented** |
...
| M5 | Memory review/edit/delete/stale workflows and bounded transcript retention | Human can correct or remove memories; retention policy is tested | 🚧 In Progress · `ca memory stale <id>` toggle implemented; no review UI yet, no retention/TTL policy |
