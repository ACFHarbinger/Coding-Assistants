...
| S5 | Orchestration and storage policy | Relocate standing Policy controls; persist ordinary and Advanced policy scopes, safe confirmation defaults, auto-enrollment, budgets, tool/sandbox settings, retention, non-destructive export, and backup settings; current hub flows honor them. | 🚧 **In Review** · #131 policy is persisted, exposed, and now enforced by the live auto-enrollment/export/sandbox paths; Settings-window UI wiring remains |
