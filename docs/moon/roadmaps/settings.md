...
- S5: [#131](https://github.com/ACFHarbinger/Coding-Assistants/issues/131) · `OrchestrationPolicy`/`OrchestrationOverride`, typed commands, the composed wake-policy editor, and budget commands are implemented; the three returned enforcement points consume the persisted policy (`send_tagged_message` refuses new-identity auto-enrollment, `hub_export_markdown`/`hub_export_markdown_git` refuse when export is disabled, `hub_start_harness`/`hub_inject_harness` refuse `vibe` under `Strict` sandbox policy); and `SettingsApp.tsx`'s Orchestration tab now exposes every field — standing wake confirmation, confirm-new-enrollment, confirm-broadcast, auto-enrollment-allowed, export-enabled, sandbox strictness (with Global/workspace scope toggle, Inherited/Override pills, and Reset to Global), retention days, and per-agent budgets — with no secret inputs. Awaiting Chat/Codex review.
