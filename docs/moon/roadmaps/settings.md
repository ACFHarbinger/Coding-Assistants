...
| S5 | Orchestration and storage policy | Relocate standing Policy controls; persist ordinary and Advanced policy scopes, safe confirmation defaults, auto-enrollment, budgets, tool/sandbox settings, retention, non-destructive export, and backup settings; current hub flows honor them. | 🚧 **In Review** · #131 backend + typed commands; Settings-window UI wiring is follow-up |
...
- S5: [#131](https://github.com/ACFHarbinger/Coding-Assistants/issues/131) · implemented: `OrchestrationPolicy`/`OrchestrationOverride` in `crates/hub::settings` (confirm-new-enrollment, confirm-broadcast, auto-enrollment-allowed, sandbox strictness, retention days, export-enabled), typed commands in `src-tauri/src/hub/commands/settings.rs` (`settings_update_orchestration`, `settings_set_retention_days`), a composed `settings_get_standing_policy`/`settings_set_confirm_wakes` view over the existing Hub `WakePolicy` (storage stays there — every C10-C13 wake path already reads it), and `settings_list_agent_budgets`/`settings_set_agent_budget` over the existing `agent_budgets` table; awaiting Chat/Codex review. Wiring an Orchestration/Advanced tab into the Settings window is follow-up UI work, not in this backend-only slice.
