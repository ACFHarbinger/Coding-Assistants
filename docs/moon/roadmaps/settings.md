...
| S5 | Orchestration and storage policy | Relocate standing Policy controls; persist ordinary and Advanced policy scopes, safe confirmation defaults, auto-enrollment, budgets, tool/sandbox settings, retention, non-destructive export, and backup settings; current hub flows honor them. | 🚧 **Partial** · #131 has persistence, enforcement, and Settings UI; legacy Shared Hub policy controls still need relocation/unification |
...
- S5: [#131](https://github.com/ACFHarbinger/Coding-Assistants/issues/131) · persistence, typed commands, runtime enforcement, and the Orchestration tab are implemented. Review returned the final relocation: remove or redirect the duplicated Shared Hub policy editor and surface the legacy `allow_auto_wake` policy through the unified Settings flow, with regression coverage. No secret inputs are permitted.
