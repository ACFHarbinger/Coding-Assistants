...
| S5 | Orchestration and storage policy | Relocate standing Policy controls; persist ordinary and Advanced policy scopes, safe confirmation defaults, auto-enrollment, budgets, tool/sandbox settings, retention, non-destructive export, and backup settings; current hub flows honor them. | 🚧 **Partial** · #131 persists and exposes the policy; runtime enforcement and Settings-window wiring remain |
...
- S5: [#131](https://github.com/ACFHarbinger/Coding-Assistants/issues/131) · `OrchestrationPolicy`/`OrchestrationOverride`, typed commands, the composed wake-policy editor, and budget commands are implemented. Review returned the slice for runtime enforcement: auto-enrollment, export permission, and sandbox strictness must be read by their live paths, with focused tests. Wiring an Orchestration/Advanced tab into the Settings window remains follow-up UI work.
