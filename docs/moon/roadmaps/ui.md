...
| U10 | Slack team chat | Slack Chat & Memory is the sole human/agent conversation surface; Orchestrate is role/team setup and Remote Control | 🚧 **Partial** · Panel in `c9932ac`; Grok follow-through: private DMs (`2ab31c7`), scroll-pin + jump-to-latest (`f16e862`, `947a43d`), Enter-to-send, persisted roster + team-wide wakes, enroll/unenroll. Claude: Edit/Delete (CA-106/109) plus a discoverable click-based **⋯** actions button and a menu self-close race fix (`29782ce`). Chat: in-context replies (CA-114). Live presence polish remains. |
