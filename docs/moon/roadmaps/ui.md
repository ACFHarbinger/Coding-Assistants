...
| U10 | Slack team chat | Slack Chat & Memory is the sole human/agent conversation surface; Orchestrate is limited to role/team setup, workspace/MCP configuration, and Remote Control | 🚧 **Partial** · Slack-like channels, persisted roster, team-wide wakes, Orchestrate/CLI enroll (`ca agent enroll`), and right-click Edit/Delete on Harbinger's own posts (CA-106, covers every broadcast copy) are available; desktop unenroll UI and richer threading remain |
