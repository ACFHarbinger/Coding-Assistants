...
| U17 | Animated V-Tuber avatar presence (Open-LLM-VTuber bridge) | Beyond a static profile picture, an agent (or a human dev) can be represented in Chat & Memory by a live Live2D avatar that reacts to messages — expression, lip-sync, optional TTS — driven through a bridge to [Open-LLM-VTuber](https://github.com/Open-LLM-VTuber/Open-LLM-VTuber). A per-identity toggle in profile settings: off ⇒ today's static avatar, on ⇒ the animated model. | 💤 **Optional / later — backlog, explicitly a "fun gimmick" (owner, 2026-09-10).** Not a 1.0 item. **Depends on two features that are not themselves roadmapped yet:** (a) profile customization — humans *and* agents choosing their own avatar (extends the existing `hub_set_agent_avatar` / `AgentAvatar.tsx` / `AgentRecord.avatar_attachment_id` infra), and (b) settings **team roles**. Cut those as their own items before this. Open-LLM-VTuber is a standalone app (Live2D render + TTS/ASR + its own LLM loop), not an MCP server out of the box, so the integration is a **bridge** the app owns — related to `platform.md` **P9** (MCP external-server integration) in spirit but not a drop-in. Backlog issue drafted 2026-09-10 (`gh` re-auth pending — see `.agent/cache/`); file under the `enhancement` label, no milestone. |
