...
| U23 | System resource monitor dashboard (CPU/RAM/Swap/Disk/GPU/VRAM) | Running multiple agent-team harnesses concurrently is resource-intensive, so a new tab shows an interactive, live-updating dashboard of total **and** per-process resource usage: total + per-core CPU, RAM, swap, disk (space + I/O), and GPU/VRAM utilization — at minimum attributing spikes to the harness processes this app itself manages. | ✅ **Landed** in `main` (Muse, Codex-reviewed PASS). New `commands/system/` module: `system_snapshot` — total + per-core CPU (freq where available), RAM, swap, per-volume disk space + I/O counters, NVIDIA GPU/VRAM via an `nvidia-smi` subprocess (explicit argv, background thread + timeout, mirrors `hub::github::run_gh`; `available:false` + detail on any other GPU/no GPU), per-process CPU/mem with managed-harness PIDs attributed via `list_harness_sessions` resolved against the live `sysinfo` process table. `sysinfo 0.38.4` (MIT) — new dependency, reviewed against `DEPENDENCY_POLICY.md`. Shared Hub **System** tab beside Board/Branches: 3s poll with an overlap guard, CPU+mem sparklines (60 samples), per-volume I/O rates from consecutive snapshots, manual refresh, error card. Read-only v1 — no process control from the UI. Cross-referenced in `dashboard.md` D7. Issue [#316](https://github.com/ACFHarbinger/Coding-Assistants/issues/316). |
