...
| U20 | Profile customization: rename + a Settings-surfaced profile view | A human or agent identity's display name is editable (today it is set once at roster-seed time and never again), and profile management — avatar + display name, for **any** roster identity including your own `human` row — has a dedicated home in Settings, not only reachable inline from wherever `AgentAvatar` happens to render in Chat & Memory. | 🆕 **Planned** (owner, 2026-09-11). **Smaller than it looks — most of the infra already exists and works today:** `hub_set_agent_avatar`/`hub_clear_agent_avatar` + `AgentAvatar.tsx`'s pick/crop/set/clear flow already operate on **any** `agents.id`, `human` included (it is a roster row like any other, `("human", "Human")`), and already renders in the Messager sidebar/header/composer/stream — so avatar customization for humans and agents is functionally done, just not centrally surfaced. The actual gap: **(a)** no rename capability at all — `agents.display_name` is only ever set by the roster-seed upsert (`crates/hub/src/store/agents/mod.rs`), no command or UI edits it after; add a typed `hub_set_agent_display_name` command (name-collision + empty-name validation, an audit event like the rest of Settings) and wire an inline edit next to the avatar wherever it's shown, or at minimum in the new Settings surface; **(b)** a Settings tab (or a section of the existing Agents tab family) listing every roster identity with its avatar + editable display name in one place, instead of only encountering the picker incidentally in Messager. Depends on nothing else; unblocks half of U17's stated prerequisite. Issue [#314](https://github.com/ACFHarbinger/Coding-Assistants/issues/314). |
| U21 | Team role assignment | Every roster identity (human or agent) can be given a role label — mirroring what this team already does informally in `AGENT_BUS.md` prose ("Claude = team lead", "Codex = review lead only") — assignable and editable from Settings, and shown as a badge wherever the identity is displayed (Messager sidebar/header, `HarnessReadinessPanel`, U18's future Kanban cards). | 🆕 **Planned** (owner, 2026-09-11). New ground, no existing infra to extend: needs a `role: Option<String>` (or a small fixed enum — **lead / reviewer / implementer / observer**, plus a free-text custom label — a real design call for whoever picks this up, not decided here) column on `agents`, a typed `hub_set_agent_role` command with its own audit event, and a Settings UI (same identity list as U20, natural to build together) to assign/clear it. Purely descriptive in v1 — no behavior gates on role (e.g. "only a lead can merge") the way the human coordinator currently enforces that by convention alone; that's a possible v2, not in scope here. Issue [#315](https://github.com/ACFHarbinger/Coding-Assistants/issues/315). |
