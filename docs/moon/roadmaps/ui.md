...
| U15 | Tiled harness-terminal grid in Orchestrate | Multiple harness CLIs run inside the app's own interactive terminals, tiled in a resizable grid (KDE-Konsole-style split view): drag splitters to push/pull pane sizes, drag a pane to re-arrange, layout persisted per workspace. The enabler for operating the whole team from inside the app rather than N external Konsole tabs + the markdown bus. | 🚧 **Slice 1 landed** (`2cb9c89`, [#296](https://github.com/ACFHarbinger/Coding-Assistants/issues/296)) · Epic [#295](https://github.com/ACFHarbinger/Coding-Assistants/issues/295). **Hand-rolled**, no tiling-library dependency (`react-mosaic`/`dockview` rejected — `DEPENDENCY_POLICY.md` §1 + the xterm mount-lifecycle constraint). Landed: pure `terminalGrid/layoutTree.ts` (binary row/col split tree — `first`/`second`/`ratio` — `computeRects`, auto-balancing `insertLeaf`, `removeLeaf` with sibling promotion, clamped `resizeSplit`, schema-validated JSON serialize) + `HarnessTerminalGrid.tsx` keeping every open `EmbeddedTerminal` mounted in a flat layer keyed by harness id and positioned by a CSS rect (resize never remounts a terminal), pointer-capture splitter push/pull resize, a 6-harness "+ add pane" palette reusing `hub_relaunch_harness_embedded`, per-pane close → `pty_kill`, per-workspace `localStorage` persistence with a Reset control, and an Orchestrate `Setup & Config` / `Terminal Grid` sub-view toggle + "Open terminal grid →" button in `HarnessReadinessPanel`. `npm test` 61/61, `npm run build` clean, no backend files touched, all files ≤500 LoC. **In flight**: drag-a-pane-header rearrange (edge = re-split, centre = swap) + maximize/restore — slice 2, [#297](https://github.com/ACFHarbinger/Coding-Assistants/issues/297) (Gemini), adds pure `moveLeaf` / `swapLeaves` to `layoutTree.ts` and a title-bar pointer-drag + drop-zone overlay on top of it, keeping the no-remount invariant. **Out of the epic**: tabbed groups, tear-out, `bash` panes, multi-instance. The layout model is intended to be reusable by the `ca tui` client (U7). Extends C14.5's Orchestrate harness UX; distinct from the coordination-substrate migration itself (`communication.md` C13/C15). |
