...
| U23 | System resource monitor dashboard (CPU/RAM/Swap/Disk/GPU/VRAM) | Running multiple agent-team harnesses concurrently is resource-intensive, so a new tab shows an interactive, live-updating dashboard of total **and** per-process resource usage: total + per-core CPU, RAM, swap, disk (space + I/O), and GPU/VRAM utilization — at minimum attributing spikes to the harness processes this app itself manages. | 🆕 **Planned — backlog** (owner, 2026-09-12). Needs: (a) a new `commands/system/` module — likely `sysinfo` for CPU/RAM/swap/disk/process enumeration (evaluate against `DEPENDENCY_POLICY.md` before adding), polled on a cheap local interval, no metered probes; (b) a GPU/VRAM path — `nvidia-smi --query-gpu=... --format=csv` subprocess at minimum (NVIDIA), same explicit-argv/background-thread/timeout pattern as the existing `gh` CLI helpers (`hub::github::run_gh`) and quota adapters, best-effort for AMD/Intel; (c) per-process attribution at least for this app's own managed harness PIDs (reuse `hub::proc::list_process_lines()` / the `pid_alive` reconcile from P3); (d) a new Shared Hub or Orchestrate tab with live meters/charts, consistent placement with U18 (Board)/U19 (Branches). Read-only v1 — no process control (kill/renice/affinity) from the UI. Cross-referenced in `dashboard.md` D7. Backlog issue [#316](https://github.com/ACFHarbinger/Coding-Assistants/issues/316) (`enhancement`, no milestone). **Landed 2026-09-12 (Muse, ready for review):** `commands/system/` + `system_snapshot` + Shared Hub System tab (3s poll, sparklines, I/O rates); sysinfo 0.38 (MIT); 5 backend + 4 tab tests. |
| U24 | Saved workspaces, linked to a Work Session Chat, selectable from Workspace Root | Today "Workspace Root" (Orchestrate) is a single free-form path with no durable link to a Work Session Chat (U11). The user can save multiple named workspaces (name + absolute path), associate each with a Work Session Chat, and select the current workspace from the Workspace Root section — picking one sets `config.work_dir` and, if it has a linked session, also focuses Chat & Memory on that session (mirroring U11's existing `selectWorkSession` pairing for "Load team chat"). | 🆕 **Planned — backlog** (owner, 2026-09-12). Needs: (a) a durable `workspaces` table in Hub SQLite (`id`/`name`/`path`/`created_at`) — `hub_list_workspaces`/`hub_save_workspace`/`hub_delete_workspace`, not `localStorage`-only (the existing `ca.workspaceRoot` single value in `hubState.ts`/`App.tsx` is the thing this replaces as the source of truth, though it can stay as a last-used-path fallback); (b) a nullable `workspace_id` FK on `work_sessions` (soft migration, same pattern as U21's `role` column) + an optional `workspaceId` param on `hub_create_work_session` — existing sessions stay unscoped (`NULL`), fully backwards compatible; (c) rework the Workspace Root section in `ConfigPanel.tsx`: a saved-workspace picker (name + path sub-label) alongside the existing free-form path/Browse/Switch controls for ad hoc paths, plus add/rename/delete affordances ("Save current as…" is the natural add path). Out of scope v1: auto-creating a session per new workspace (explicit action only), multiple workspaces open at once. Backlog issue [#317](https://github.com/ACFHarbinger/Coding-Assistants/issues/317) (`enhancement`, no milestone). |
| U25 | Chat-platform bot remote client (Discord/Telegram/Slack-style) | A bot on a chat platform a user already has open (the way Grok itself ships as a bot on X/Discord/Telegram) becomes another CA remote-control client surface, alongside the existing Android app (U5): watch team events, approve/reject wakes, and send messages to the agent team from a chat command, without the desktop app or phone in front of you. | 🆕 **Planned — backlog** (owner, 2026-09-12). Needs: (a) a bot process/adapter for one platform first — Discord is the likely first target (mature bot API, slash commands, threads map naturally onto work-session chats); Telegram/Slack later, same category; (b) read path surfacing Hub events (messages, wakes, task status) into a bound channel/DM; (c) write path for approve/reject + send-message from a chat command, mirroring U5's Android `DashboardScreen` scope (events + approve/reject), not full task configuration; (d) a pairing/token step scoping the bot to one user's Hub instance, not an open relay — reuses P12's credential resolver for the bot token. Out of scope v1: multi-user/shared-server deployments, task creation or role config from the bot. Distinct from `platform.md` **P15** (xAI's own Grok Bot API as a model *provider* — this is the client-surface half). Backlog issue [#319](https://github.com/ACFHarbinger/Coding-Assistants/issues/319) (`enhancement`, no milestone). |
