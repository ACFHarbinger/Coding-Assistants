...
| P9 | MCP external-server integration and promotion of frequently used tools | External MCP remains the lean default; promoted tools have security tests | 🚧 **Partial** · `hub::mcp::render_merged`/`render_replacing` render arbitrary server entries into each client config; `hub::mcp::creative` is a per-workspace catalog + `apply_to_workspace` for the 7 (now 8, incl. Ableton) creative-tool bridges. **In flight (2026-09-08):** generalise `creative` into a shared **external-MCP-server registry** (#272, Claude) that also carries non-creative servers, then register the two Perplexity servers on it — official **`@perplexity-ai/mcp-server`** (npx/Node, `PERPLEXITY_API_KEY`, tools `perplexity_{search,ask,research,reason}`, #276) and the subscription-session **`perplexity-web-mcp-cli`** (`pwm-mcp`, Python/uv, `pwm login` OTP session token, ~20 `pplx_*` tools, #277). Both stdio; both need a Settings toggle (#278, Gemini). |
...
- **P9** — Perplexity official API MCP ([#276](https://github.com/ACFHarbinger/Coding-Assistants/issues/276), owner: Grok).
- **P9** — Perplexity subscription web MCP ([#277](https://github.com/ACFHarbinger/Coding-Assistants/issues/277), owner: Grok).
