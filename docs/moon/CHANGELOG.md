...
- Handles the balance API's **string-typed** monetary fields (`"12.34"`) with explicit parsing, and surfaces the dollar balance through a new optional `ProviderQuota.balance` field rendered distinctly in the Usage tab's `QuotaChart` (account-balance row) instead of forcing it into percent windows.
- Added a compact `QuotaStatusStrip` in the Messager sidebar's agents/status area mirroring DeepSeek + OpenCode Go usage from `hub_get_provider_quotas` on a 60s poll — a sane interval, not tight, so the slow `/ogc-usage` backend call is not hammered.
