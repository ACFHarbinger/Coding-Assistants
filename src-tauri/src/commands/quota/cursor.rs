...
const DETAIL: &str = "Cursor Agent exposes JSON authentication/account metadata but no machine-readable plan usage or remaining request limits; check Cursor Dashboard → Usage";
...
mod tests {
    ...
    fn reports_the_typed_unavailable_row() {
        ...
        assert!(quota
            .detail
            .as_deref()
            .is_some_and(|detail| detail.contains("no machine-readable plan usage")));
    }
