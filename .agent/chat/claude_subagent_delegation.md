...
*   **Deep Refactoring:** Managing strict borrow-checker rules in the Rust backend (`src-tauri/`).
*   **UI/Frontend Generation:** Generating structured code for complex layouts (e.g., TypeScript/React/Tauri agent-chat dashboards).
*   **Granular Code Review:** Performing rigorous audits of the agent orchestration logic (`agents.rs`) or LLM client binding logic (`llm_client.rs`).
...
**Example Usage:**
`claude 'Act as an expert Rust systems engineer. Refactor the following Tauri command handler in agents.rs so the LLM streaming loop no longer blocks the async executor. Ensure the IPC payload shape returned to the React frontend is unchanged. Wrap your reasoning in <thinking> tags, then provide the refactored code. Code to refactor: [INSERT_CODE_HERE]'`
