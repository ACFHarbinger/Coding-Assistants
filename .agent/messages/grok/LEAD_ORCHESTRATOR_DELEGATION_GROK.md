...
### Objective
Wire process detector (`detect_agent_processes` Tauri command) into the Slack-like channel sidebar and agent roster to display real-time active agent process status (`ONLINE` / `IDLE` / `OFFLINE`) for Grok, Claude, Codex, and Gemini.
